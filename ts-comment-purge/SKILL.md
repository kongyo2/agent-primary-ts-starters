...
Removal is safe by construction, so no preview step is needed before running it: deletion follows the AST's comment ranges (strings, template literals, regexes, and JSX text are never touched), writes are atomic (temp file + rename), UTF-8/UTF-16 encodings and BOMs are preserved, and after removal the tool re-scans each file and **errors without modifying it** if the result doesn't match expectations. Files with invalid encoding are reported and left unchanged.
...
Only when the user explicitly asks to also remove the protected classes:
...
- **Structurally protected comments** — a comment whose removal would shift a next-line directive (`@ts-expect-error`, `eslint-disable-next-line`) onto the wrong line, or would newly activate a position-dependent pragma (Prettier's `@format`, Bun's `// @bun` — valid only as the file's first comment). The tool resolves these cases itself: comment-only lines are removed whole, trailing comments take their leading whitespace with them, and a space is inserted where block-comment removal would join tokens (`a/* x */b` → `a b`).
...
| `--remove-directives` / `--remove-legal` | With `--remove`: also delete the protected classes |
